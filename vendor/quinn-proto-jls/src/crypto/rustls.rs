...
impl crypto::Session for TlsSession {
    ...
    fn handshake_data(&self) -> Option<Box<dyn Any>> {
        ...
        let (jls_authenticated, jls_user) = match self.inner.jls_state() {
            rustls::jls::JlsState::AuthSuccess(user) => {
                (Some(true), Some(user.user_iv.clone()))
            }
            rustls::jls::JlsState::AuthFailed(_) => (Some(false), None),
            rustls::jls::JlsState::NotAuthed | rustls::jls::JlsState::Disabled => (None, None),
        };
        Some(Box::new(HandshakeData {
            ...
            jls_authenticated,
            jls_user,
            jls_upstream_addr: self.inner.jls_upstream_addr(),
            #[cfg(feature = "__rustls-post-quantum-test")]
...
pub struct HandshakeData {
    ...
    /// Whether the TLS session completed JLS authentication.
    ///
    /// `None` means JLS was not enabled for this session.
    pub jls_authenticated: Option<bool>,
    /// The authenticated JLS user identifier, when authentication succeeded.
    pub jls_user: Option<String>,
    /// Configured camouflage upstream for failed JLS handshakes, if any.
